    ...
    <div className="max-w-5xl mx-auto bg-white text-zinc-800">
      <div className="grid grid-cols-3">
        <div className="col-span-1 py-10">
          {/* Profile Image */}
          {data.professional_info?.image && (
            <div
              className="w-36 h-36 rounded-full mx-auto mb-2 flex items-center justify-center"
              style={{
                backgroundColor: accentColor, // outer ring
              }}
            >
              <div
                className="w-32 h-32 rounded-full flex items-center justify-center"
                style={{
                  backgroundColor: accentColor + "33", // 👈 faded inner bg
                }}
              >
                <img
                  src={
                    typeof data.professional_info.image === "string"
                      ? data.professional_info.image
                      : URL.createObjectURL(data.professional_info.image)
                  }
                  alt="Profile"
                   style={{
    border: `1px solid ${accentColor}`,   // 👈 thickness control here
        ...
        <div className="col-span-2 flex flex-col justify-center py-10 px-8">
          ...
          <p className="uppercase text-zinc-600 font-medium text-sm tracking-widest">
            {data?.professional_info?.profession || "Profession"}
          </p>
        </div>
            ...
            <section className="mt-8">
              ...
              <div className="space-y-3 text-sm">
                {data.achievements.map((ach, index) => (
                  <div key={index}>
                    <p className="font-semibold uppercase">{ach.title}</p>
                    <p className="text-zinc-600">{ach.organization}</p>
                    <p className="text-xs text-zinc-500">
                      {formatDate(ach.date)}
                    </p>
                    {ach.description && (
                      <p className="text-zinc-700">{ach.description}</p>
                    )}
                    {ach.link && (
                      <a
                        href={ach.link}
